...
class SearchFragment : Fragment() {
    ...
    private fun viewInit() {
        searchViewModel.mainListUiState.observe(viewLifecycleOwner) {
            when (it) {
        ...
        searchViewModel.showProgress.observe(viewLifecycleOwner) { showProgress ->
            binding.groupLoading.visibility = if (showProgress) {
                View.VISIBLE
            } else {
                View.GONE
            }
        ...
        searchQueryViewModel.searchQuery.observe(viewLifecycleOwner) {
            searchViewModel.updateKeyword(it)
        }

        filterStatusViewModel.filterUiState.observe(viewLifecycleOwner) {
